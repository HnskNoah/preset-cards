 ...
 * 仅组织 v3 节点。根 = base + baseId 无对应 v3 父的孤立 delta；同层保持原数组相对顺序。
 */
export function buildProfileForest(profiles: PresetProfile[]): ProfileTreeNode[] {
    const supported = profiles.filter((profile) => isPromptBaseProfile(profile) || isPromptDeltaProfile(profile));
    ...
    // 根候选 = v3 base + baseId 无对应 v3 父的孤立 delta。
