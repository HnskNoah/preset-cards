...
- **扩展配置随 profile（扩展捕获）**：profile 可记录预设 extensions 的开关与数组条目增删（正则脚本、SPreset 绑定、tavern_helper 脚本等），加载 profile 时一并应用；在注册 profile 激活期间的扩展变更同样被自动捕获。加载时**开关沿父链继承**（祖先 → 自身，同名键自身优先），结构性增删仅应用本层。
