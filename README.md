...
- **预设更新迁移（实验性，使用前请导出备份）**：支持把旧预设的整棵 Base/Delta profile 树 rebase 到更新版预设；按 `identifier` 优先匹配，identifier 变化但内容指纹唯一一致时自动重映射；冲突在图形化迁移编辑器中处理，支持旧出厂值 / 新版值 / 我的修改 / 手动编辑四种选择。顺序策略支持“保留我的顺序”和“跟随新版顺序”，新条目可跟随新版出厂挂载或保持未挂载；可选“从来源预设带入”会追加目标缺失且被 profile 引用的 prompt 定义，并按 id 或 `scriptName` 合并来源正则。迁移执行为追加式：旧预设保持原样，结果追加到目标预设；未解决冲突、孤立 Delta、过期冲突裁决或目标槽位漂移都会阻断应用，避免静默改坏目标。
