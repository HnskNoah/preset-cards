...
describe('saveMeta 持久化统一机制', () => {
    it('onMetaPersisted 在 persistMetaTransaction 成功后触发', async () => {
        ...
        const ok = await persistMetaTransaction((m) => ({ ...m, description: 'x' }), 'Midnight', idx);
        expect(ok).toBe(true);
    ...
    it('patch.extensions 不回滚本次 meta 容器（迁移「带入正则」场景）', async () => {
        ...
        const ok = await persistMetaTransaction((m) => ({ ...m, profiles: [{ ...(m.profiles?.[0] ?? {}), id: 'p-new', kind: 'prompt_base', formatVersion: 3, name: 'N', prompts: [] } as any] }), 'Midnight', idx, { patch });
        expect(ok).toBe(true);
    ...
    it('saveMetaMerged（编辑器提交路径）也触发 onMetaPersisted → 注册对账（L17）', async () => {
        ...
        await saveMetaMerged('Midnight', idx, (m) => ({ ...m, description: 'y' }));
        expect(seen).toEqual([['Midnight', 0]]);
    });

    it('合并窗口内多次保存合并为一次，后写变换的最终值胜出', async () => {
        vi.useFakeTimers();
    ...
    it('合并窗口内多次变换依次重放于最新状态（丢失更新回归）', async () => {
        vi.useFakeTimers();
        addPreset('Replay', { prompts: [], extensions: { preset_cards: { description: '', models: [], profiles: [], bgImage: '' } } });
        const idx = openai_settings.length - 1;
        // 两个并发操作各自追加一个 profile：旧「末次整体胜出」会丢掉先到的追加
        const p1 = saveMeta('Replay', idx, (m) => ({ ...m, profiles: [...m.profiles, { id: 'a', kind: 'prompt_base', formatVersion: 3, name: 'A', prompts: [] } as any] }));
        const p2 = saveMeta('Replay', idx, (m) => ({ ...m, profiles: [...m.profiles, { id: 'b', kind: 'prompt_base', formatVersion: 3, name: 'B', prompts: [] } as any] }));
        await vi.advanceTimersByTimeAsync(400);
        await Promise.all([p1, p2]);
        const fetchMock = fetch as unknown as ReturnType<typeof vi.fn>;
        expect(fetchMock).toHaveBeenCalledTimes(1);
        const body = JSON.parse(fetchMock.mock.calls[0][1].body as string);
        expect(body.preset.extensions.preset_cards.profiles.map((p: any) => p.id)).toEqual(['a', 'b']);
        // 落盘结果同步回活 extensions（真实写回，非快照写回）
        const live = (openai_settings[idx] as any).extensions.preset_cards.profiles.map((p: any) => p.id);
